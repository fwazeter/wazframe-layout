...
export function hasValue( props ) {
    return props.attributes.style?.position?.inlineStart !== undefined;
}
...
export function InlineStartEdit( props ) {
    const {
        attributes: { style },
        setAttributes,
    } = props
    ...
    return (
        <>
            <UnitControl
                value={ style?.position?.inlineStart }
                onChange={ onChange }
                label={ __( 'X Start', namespace ) }
                units={ units }
            />
        </>
    );
